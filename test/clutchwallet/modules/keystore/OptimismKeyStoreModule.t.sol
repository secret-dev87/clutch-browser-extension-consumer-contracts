...
contract OptimismKeyStoreModuleTest is Test, MockKeyStoreData {
    ...
    function setUp() public {
        ...
        knownStateRootWithHistory = new OpKnownStateRootWithHistory(
            address(mockL1Block)
        );
        keystoreProofContract = new KeystoreProof(
            0xACB5b53F9F193b99bcd8EF8544ddF4c398DE24a3,
            address(knownStateRootWithHistory)
        );
        optimismKeyStoreModule = new KeyStoreModule(
            address(keystoreProofContract)
        );
        ...
        walletL1Slot = KeyStoreSlotLib.getSlot(
            initialKeyBytes32,
            initialGuardianHash,
    ...
    function deployWallet() private {
        ...
        bytes32 l1Slot = stdstore
            .target(address(optimismKeyStoreModule))
            .sig("l1Slot(address)")
            .with_key(address(clutchWallet))
            .depth(0)
            .read_bytes32();
    ...
    function hackModifyKeyStoreSLot() private {
        ...
        stdstore
            .target(address(keystoreProofContract))
            .sig("l1SlotToSigningKey(bytes32)")
            .with_key(walletL1Slot)
            .depth(0)
    ...
    function test_setUp() public {
        ...
        assertEq(
            _modules[0],
            address(optimismKeyStoreModule),
    ...
    function proofL1KeyStore() internal {
        ...
        knownStateRootWithHistory.insertNewStateRoot(
            TEST_BLOCK_NUMBER,
            blockInfoParameter
        );
    ...
    function test_setUpWithKeyStoreValueExist() public {
        ...
        proofL1KeyStore();
        deployWallet();
        assertEq(clutchWallet.isOwner(walletOwner), false);
        assertEq(clutchWallet.isOwner(newOwner), true);
    }

    function test_keyStoreSyncWithModule() public {
        deployWallet();
        assertEq(clutchWallet.isOwner(walletOwner), true);
        assertEq(clutchWallet.isOwner(newOwner), false);
        proofL1KeyStore();
        vm.startPrank(walletOwner);
        optimismKeyStoreModule.syncL1Keystore(address(clutchWallet));
        assertEq(clutchWallet.isOwner(walletOwner), false);
        assertEq(clutchWallet.isOwner(newOwner), true);
        vm.stopPrank();
    }
