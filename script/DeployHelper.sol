...
library NetWorkLib {
    function getNetwork() internal view returns (Network network) {
        if (block.chainid == 1 || block.chainid == 1337) {
...
abstract contract DeployHelper is Script {
    address deployer;
    uint256 privateKey;
    address internal constant SINGLE_USE_FACTORY_ADDRESS =
    ...
    address internal constant EMPTY_ADDRESS =
        0x0000000000000000000000000000000000000000;
    bytes emptyBytes;
    address internal ENTRYPOINT_ADDRESS =
    ...
    function deploy(
        ...
        if (calculatedAddress.code.length == 0) {
            ...
            require(
                calculatedAddress == acutalAddress,
                "create2 address mismatch"
            );
            console.log(
                pad("Deploying", 10),
                pad(name, 33),
                pad(LibString.toHexString(acutalAddress), 63)
            );
        } else {
            console.log(
                pad("Finding", 10),
                pad(name, 33),
                pad(LibString.toHexString(calculatedAddress), 63)
            );
        }
    ...
    function deploySingletonFactory() internal {
        if (address(SINGLETON_FACTORY).code.length == 0) {
            ...
            sendEthInputs[4] = LibString.toHexString(
                SINGLE_USE_FACTORY_ADDRESS
            );
