...
contract PaymasterDeployer is Script, DeployHelper {
    ...
    function run() public {
        ...
        require(
            paymasterOwnerPrivateKey != 0,
            "PAYMASTER_OWNER_PRIVATE_KEY not provided"
        );
    ...
    function delpoyLocal() private {
        address testUsdc = deploy(
            "TestUsdc",
            bytes.concat(type(TokenERC20).creationCode, abi.encode(6))
        );
        address testOracle = deploy(
            "TestOracle",
            bytes.concat(
                type(TestOracle).creationCode,
                abi.encode(190355094900)
            )
        );
        address paymaster = deploy(
            "Paymaster",
            bytes.concat(
                type(ERC20Paymaster).creationCode,
                abi.encode(
                    ENTRYPOINT_ADDRESS,
                    paymasterOwner,
                    clutchwalletFactory
