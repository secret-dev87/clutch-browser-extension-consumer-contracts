...
contract KeystoreDeployer is Script, DeployHelper {
    address private constant OP_L1_BLOCK_ADDRESS =
        0x4200000000000000000000000000000000000015;

    //arb inbox contract address, https://developer.arbitrum.io/for-devs/useful-addresses
    address private constant ARB_ONE_INBOX_ADDRESS =
        0x4Dbd4fc535Ac27206064B68FfCf827b0A60BAB3f;
    address private constant ARB_GOERLI_INBOX_ADDRESS =
    ...
    function run() public {
        ...
        require(
            ...
            "proxyAdminAddress not provided"
        );
    ...
    function mainnetDeploy() private {
        require(
            address(SINGLETON_FACTORY).code.length > 0,
        ...
        address keyStoreModule = deploy(
            "KeyStoreModule",
            bytes.concat(
                type(KeyStoreModule).creationCode,
                abi.encode(keyStore)
            )
        );
    ...
    function arbDeploy() private {
        ...
        require(
            l1KeyStoreAddress != address(0),
            "L1_KEYSTORE_ADDRESS not provided"
        );
        require(
            address(SINGLETON_FACTORY).code.length > 0,
            "singleton factory not deployed"
        );
        arbL1KeyStorePassingAddress = vm.envAddress(
            "ARB_L1_KEYSTORE_PASSING_ADDRESS"
        );
        require(
            arbL1KeyStorePassingAddress != address(0),
            "ARB_L1_KEYSTORE_PASSING_ADDRESS not provided"
        );
        ...
        address arbKnownStateRootWithHistory = deploy(
            ...
            bytes.concat(
                type(ArbKnownStateRootWithHistory).creationCode,
                abi.encode(EMPTY_ADDRESS, proxyAdminAddress)
            )
        );
        ...
        address keystoreProof = deploy(
            "KeystoreProof",
            bytes.concat(
                type(KeystoreProof).creationCode,
                abi.encode(l1KeyStoreAddress, arbKnownStateRootWithHistory)
            )
        );
        ...
        ITransparentUpgradeableProxy(keyStoreProxy).upgradeTo(keyStoreModule);
        // setup l1 target
        ArbKnownStateRootWithHistory(arbKnownStateRootWithHistory)
            .updateL1Target(arbL1KeyStorePassingAddress);
        writeAddressToEnv(
            "ARB_KNOWN_STATE_ROOT_WITH_HISTORY_ADDRESS",
            arbKnownStateRootWithHistory
        );
    }
    ...
    function opDeploy() private {
        ...
        address opKnownStateRootWithHistory = deploy(
            ...
            bytes.concat(
                type(OpKnownStateRootWithHistory).creationCode,
                abi.encode(OP_L1_BLOCK_ADDRESS)
            )
        );
        ...
        address keystoreProof = deploy(
            ...
            bytes.concat(
                type(KeystoreProof).creationCode,
                abi.encode(l1KeyStoreAddress, opKnownStateRootWithHistory)
            )
        );
